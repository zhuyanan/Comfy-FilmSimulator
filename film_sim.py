...
class FilmSimNode:
    ...
    def get_film_params(self, name):
        ...
        # Ensure matrix exists and has at least 12 elements (3x3 + offsets)
        if "matrix" not in p or p["matrix"] is None:
            p["matrix"] = [1,0,0,0, 1,0,0,0, 1,0,0,0]
        while len(p["matrix"]) < 12:
            p["matrix"].append(0.0)
        if "grain_base" not in p:
            p["grain_base"] = [0,0,0,0]
        while len(p["grain_base"]) < 4:
            p["grain_base"].append(0.0)
        if "sens_factor" not in p:
            p["sens_factor"] = 1.0
        # Default [Diffusion, Linear, Gamma] - ensure each channel key exists
        if "opt_r" not in p:
            p["opt_r"] = [1.0, 1.0, 1.0]
        if "opt_g" not in p:
            p["opt_g"] = [1.0, 1.0, 1.0]
    ...
    def calculate_auto_exposure(self, img_np, target_gray=0.18):
        # img_np is expected in linear space
        luma = 0.2126 * img_np[:,:,0] + 0.7152 * img_np[:,:,1] + 0.0722 * img_np[:,:,2]
        valid_mask = luma > 0.001
        if np.sum(valid_mask) > 0:
            valid_luma = luma[valid_mask]
            # use log-average to be robust to outliers
            log_avg = np.mean(np.log(np.maximum(valid_luma, 1e-8)))
            avg_luma = np.exp(log_avg)
        else:
            avg_luma = 0.18
        scale = target_gray / max(avg_luma, 1e-8)
    ...
    def kelvin_to_rgb(self, k):
        ...
        if temp <= 66:
            r = 255
            # g can go below zero for very low temps - clamp later
            g = 99.4708025861 * np.log(np.maximum(temp, 1e-6)) - 161.1195681661
            if temp <= 19:
                b = 0
            else:
                b = 138.5177312231 * np.log(np.maximum(temp - 10, 1e-6)) - 305.0447927307
        else:
    ...
    def apply_physical_white_balance(self, img, kelvin, tint):
        ...
        # tint adjusts green channel slightly (positive tint reduces green)
        tint_factor = 1.0 - (tint / 100.0)
        gain_g *= tint_factor
        # normalize to preserve overall luma
    ...
    def apply_local_contrast(self, image, strength=0.2):
        if strength <= 0:
            return image
        # operate in log-domain to preserve highlight rolloff
        log_img = np.log1p(np.maximum(image, 0.0))
        h, w = image.shape[:2]
        # kernel size scaled to image size, ensure odd and at least 1
        k = max(1, int(min(h, w) * 0.01))
        if k % 2 == 0:
            k += 1
        blurred_log = cv2.GaussianBlur(log_img, (k, k), 0)
    ...
    def apply_film(self, image, film_type, wb_temperature_K, wb_tint, auto_exposure, exposure, effect_strength, grain_power, halation_power, local_contrast, is_linear_input):
        # image: torch tensor batch with shape likely (N,H,W,3) or (N,3,H,W)
        ...
        for i in range(image.shape[0]):
            ...
            # handle channel-order variations -> ensure HxWx3
            ...
            # 1. 线性化 (if input is sRGB-like, convert to linear)
            if not is_linear_input:
                img_np = np.power(np.maximum(img_np, 0.0), 2.2)
            ...
            g = np.maximum(g, 0.0)
            b = np.maximum(b, 0.0)
            img_graded = np.dstack((r, g, b)).astype(np.float32)
            ...
            if p["sens_factor"] > 0 and halation_power > 0:
                threshold_mask = np.maximum(luma - 0.5, 0.0)
            ...
            # 8. Grain
            grain_base = p.get("grain_base", [0,0,0,0])
            if np.sum(grain_base) > 0 and grain_power > 0:
                ...
                if p["type"] == "color":
                    ...
                    noise_stack = np.dstack((gn_r_mix, gn_g_mix, gn_b_mix)).astype(np.float32)
            ...
            # ensure float32 before converting to tensor
            hdr_final = hdr_final.astype(np.float32)
            ...
            sdr_view = np.clip(sdr_view, 0, 1).astype(np.float32)
